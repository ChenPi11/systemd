...
#define REPORT_SIGN_TPM2_PERSISTENT_DIR LOCALSTATEDIR "/lib/systemd/report.sign.tpm2"
#define REPORT_SIGN_TPM2_RUNTIME_DIR RUNSTATEDIR "/systemd/report.sign.tpm2"
...
static int load_key_context(Tpm2Context *c, int dir_fd, Tpm2Handle **ret_handle) {
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to unmarshal saved signing key context: %m");

        _cleanup_(tpm2_handle_freep) Tpm2Handle *handle = NULL;
        r = tpm2_load_saved_handle_context(c, &context, /* ret_name= */ NULL, &handle);
        if (r < 0)
                return log_error_errno(r, "Failed to load saved signing key context: %m");
...
static int save_key_context(Tpm2Context *c, int dir_fd, const Tpm2Handle *handle) {
        ...
        if (r < 0)
                return log_debug_errno(r, "Failed to save signing key context: %m");

        _cleanup_free_ void *buf = NULL;
        size_t buf_sz;
        r = tpm2_marshal_saved_handle_context(context, &buf, &buf_sz);
        if (r < 0)
                return log_debug_errno(r, "Failed to marshal signing key context: %m");
...
static int vl_method_sign(
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to acquire a signing key handle: %m");

        _cleanup_(tpm2_report_freep) Tpm2Report *report = NULL;
        r = tpm2_generate_report(c, &opts, key_handle, &external_data, &report);
        if (r < 0)
                return log_error_errno(r, "Failed to generate TPM report: %m");
