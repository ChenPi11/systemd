...
static int save_sysctl_options(OrderedHashmap *sysctl_options, const char *filename) {
        ...
        r = mkdir_p(RUNSTATEDIR "/sysctl.d/", 0755);
        if (r < 0)
                return log_error_errno(r, "Failed to create directory '/run/sysctl.d/': %m");

        _cleanup_free_ char *path = path_join(RUNSTATEDIR "/sysctl.d/", filename);
        if (!path)
                return log_oom();
...
static int run(int argc, char *argv[]) {
        ...
        if (arg_revert_file) {
                _cleanup_free_ char *path = path_join(RUNSTATEDIR "/sysctl.d/", arg_revert_file);
                if (!path)
                        return log_oom();
