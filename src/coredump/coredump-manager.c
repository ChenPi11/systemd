...
#define COREDUMP_SOCKET_PATH           RUNSTATEDIR "/systemd/coredumpd/kernel"
#define COREDUMP_SOCKET_MAX_WORKERS     8u

#define WORKER_TIMEOUT_USEC (5 * USEC_PER_MINUTE)

enum {
        /* This should have a higher priority than the others, to prevent accepting new connections. */
...
static int on_worker_sigchld(sd_event_source *s, const siginfo_t *si, void *userdata) {
        ...
        /* We know the worker is dead. Let's avoid calling pidref_done_sigkill_wait() in worker_free(). */
        worker->dead = true;
        ...
        case CLD_EXITED:
                if (si->si_status == 0)
                        ...
                        log_warning("Worker ["PID_FMT"] exited with code %i.", si->si_pid, si->si_status);
                break;
...
static int manager_spawn_worker(Manager *manager, int coredump_fd) {
        ...
        r = event_add_child_pidref(manager->event, &worker->child_event_source,
                                   &worker->pidref, WEXITED, on_worker_sigchld, worker);
        if (r < 0)
                return r;
...
static int manager_exit(Manager *manager, int exit_code) {
        assert(manager);
        ...
        manager->coredump_socket = safe_close(manager->coredump_socket);
...
static int on_sigterm(sd_event_source *s, const struct signalfd_siginfo *si, void *userdata) {
        return manager_exit(ASSERT_PTR(userdata), /* exit_code= */ 0);
}
...
static int manager_init_event(Manager *manager) {
        ...
        r = manager_setup_signal(manager, event, SIGINT, on_sigterm,
                                 EVENT_PRIORITY_SIGTERM_SIGINT, "sigint-event-source");
        if (r < 0)
                return log_error_errno(r, "Failed to create SIGINT event source: %m");

        r = manager_setup_signal(manager, event, SIGTERM, on_sigterm,
                                 EVENT_PRIORITY_SIGTERM_SIGINT, "sigterm-event-source");
        if (r < 0)
                return log_error_errno(r, "Failed to create SIGTERM event source: %m");

        r = manager_setup_signal(manager, event, SIGHUP, on_sighup,
                                 EVENT_PRIORITY_SIGHUP, "sighup-event-source");
        if (r < 0)
...
static int manager_open_coredump_socket(Manager *manager) {
        ...
        WITH_UMASK(0077)
                r = RET_NERRNO(bind(fd, &sa.sa, sa_len));
        if (r < 0)
                return log_error_errno(r, "Failed to bind AF_UNIX socket at %s: %m", sa.un.sun_path);
...
static int manager_listen_fds(Manager *manager) {
        ...
        for (int i = 0; i < n; i++) {
                ...
                if (streq_ptr(names[i], "coredump-socket"))
                        ...
                        r = log_warning_errno(SYNTHETIC_ERRNO(EINVAL),
                                              "Received unexpected fd (%i: %s), ignoring.", fd, names[i]);
                if (r < 0)
...
int coredump_manager_check_requirements(int log_level) {
        ...
        NamespaceType type;
        FOREACH_ARGUMENT(type, NAMESPACE_PID, NAMESPACE_USER, NAMESPACE_TIME) {
                r = namespace_is_init(type);
                if (r < 0)
                        return log_error_errno(r, "Failed to check if we are in the initial %s namespace: %m",
                                               namespace_info[type].proc_name);
                if (r == 0)
                        return log_full_errno(log_level, SYNTHETIC_ERRNO(EOPNOTSUPP),
                                              "Running in a non-initial %s namespace.",
                                              namespace_info[type].proc_name);
        }
...
int coredump_manager(void) {
        ...
        (void) dlopen_compress_default(LOG_DEBUG);
