...
int machine_id_commit(const char *root) {
        ...
        r = chase(SYSCONF_DIR "/machine-id", root, CHASE_PREFIX_ROOT|CHASE_MUST_BE_REGULAR,
        ...
        if (r < 0)
                return log_error_errno(r, "Failed to open %s: %m", SYSCONF_DIR "/machine-id");
        ...
        _cleanup_close_ int etc_machine_id_dir_fd =
                chase_and_open_parent(SYSCONF_DIR "/machine-id", root, CHASE_PREFIX_ROOT, &etc_machine_id_filename);
