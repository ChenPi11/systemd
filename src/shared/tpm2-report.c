...
Tpm2Report *tpm2_report_free(Tpm2Report *rp) {
        if (!rp)
                return NULL;

        rp->event_log = sd_json_variant_unref(rp->event_log);
        rp->public_key = mfree(rp->public_key);
        tpm2_report_component_array_free(rp->components, rp->n_components);
        rp->components = NULL;
        rp->n_components = 0;
...
static void nvpcr_report_request_done(NvPCRReportRequest *req) {
        assert(req);

        req->name = mfree(req->name);
        sym_Esys_Free(req->public);
        req->public = NULL;
        req->handle = tpm2_handle_free(req->handle);
        req->authenticated_data = mfree(req->authenticated_data);
}
...
static int tpm2_generate_report_try(
        ...
        r = sd_varlink_connect_address(&vl, RUNSTATEDIR "/systemd/io.systemd.PCRLock");
        if (r < 0)
...
int tpm2_generate_report(
        ...
        STRV_FOREACH(s, options->nv_pcrs) {
                ...
                if (r < 0)
                        return log_debug_errno(r, "Attempt to report invalid NvPCR '%s'", name);
