# 接口名称：org.freedesktop.systemd1.Service

## 接口说明

`org.freedesktop.systemd1.Service` 是 systemd 中**服务单元（`.service`）的 D-Bus 接口**。服务单元用于管理守护进程、一次性命令及其他可执行程序的生命周期。

通过此接口，客户端可以：

- **查询服务运行状态**：主进程 PID、控制进程 PID、重启次数等
...
其中 `<service_name>` 是服务单元名称经过 D-Bus 路径转义后的形式（例如 `nginx.service` 对应 `/org/freedesktop/systemd1/unit/nginx_2eservice`）。
...
| `GuessMainPID` | `b` | 只读 | 为 `true` 时，对 `forking` 类型服务自动猜测主进程 PID（当无 PIDFile 时使用） |
| `RootDirectoryStartOnly` | `b` | 只读 | 为 `true` 时，`RootDirectory` 仅对 `ExecStart` 命令生效，其他执行命令不受影响 |
...
| `BusName` | `s` | 只读 | D-Bus 服务名称，`Type=dbus` 时 systemd 等待该服务名出现在总线上才认为启动完成 |
...
| 属性名称 | 类型 | 读写 | 说明 |
|---------|------|------|------|
| `TimeoutStartUSec` | `t` | 只读 | 启动超时时间（微秒）；超时后触发 `TimeoutStartFailureMode` 指定的动作 |
| `TimeoutStopUSec` | `t` | 只读 | 停止超时时间（微秒）；超时后强制终止进程 |
| `TimeoutAbortUSec` | `t` | 只读 | 中止超时时间（微秒）；在 `abort` 操作中等待进程退出的最长时间 |
...
| `RuntimeRandomizedExtraUSec` | `t` | 只读 | 在 `RuntimeMaxUSec` 基础上随机增加的额外时间（微秒），用于错开多个服务的超时 |
...
| `Result` | `s` | 只读 | 服务最终结果：`success` / `exit-code`（异常退出码）/ `signal`（被信号终止）/ `core-dump`（产生核心转储）/ `watchdog`（看门狗超时）/ `timeout`（启动/停止超时）/ `service-watchdog`（服务看门狗失败）等 |
| `ReloadResult` | `s` | 只读 | 最近一次重载操作的结果 |
| `CleanResult` | `s` | 只读 | 最近一次清理（`systemctl clean`）操作的结果 |
| `LiveMountResult` | `s` | 只读 | 实时挂载操作的结果 |
...
| 属性名称 | 类型 | 读写 | 说明 |
|---------|------|------|------|
| `USBFunctionDescriptors` | `s` | 只读 | USB gadget 功能描述符文件路径 |
| `USBFunctionStrings` | `s` | 只读 | USB gadget 功能字符串文件路径 |
...
| `RefreshOnReload` | `as` | 只读 | 重载时需要重新获取的资源名称列表 |

### 继承自执行上下文（ExecuteContext）的关键属性

以下属性来自 systemd 执行上下文，控制进程的运行环境，此处列出核心属性：

| 属性名称 | 类型 | 读写 | 说明 |
|---------|------|------|------|
| `ExecCondition` | `a(sasbttttuii)` | 只读 | 预检命令列表；任一命令失败则跳过服务（不视为失败） |
...
| `WorkingDirectory` | `s` | 只读 | 进程工作目录 |
| `RootDirectory` | `s` | 只读 | 根目录（chroot） |
| `User` | `s` | 只读 | 运行进程的用户名或 UID |
...
> 执行上下文还包含大量与命名空间隔离、安全加固、资源限制、能力（Capabilities）等相关的属性，此处不一一列举。
...
**说明：** 向正在运行的服务的挂载命名空间中动态添加一个绑定挂载。服务必须已启动且具有私有挂载命名空间（`PrivateMounts=yes` 或类似设置）。
...
**说明：** 向正在运行的服务的挂载命名空间中动态挂载一个磁盘镜像文件（如 squashfs、ext4 等）。
